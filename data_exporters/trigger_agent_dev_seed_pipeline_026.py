...
def trigger_seeded_downstream(*args, **kwargs) -> None:
    ...
    try:
        trigger_pipeline(
            ...
            schedule_name='Agent dev dependency - agent_dev_seed_pipeline_013 to agent_dev_seed_pipeline_026',
            verbose=False,
            _should_schedule=True,
        )
    finally:
        dependency_schedules = PipelineSchedule.repo_query.filter(
            PipelineSchedule.name == 'Agent dev dependency - agent_dev_seed_pipeline_013 to agent_dev_seed_pipeline_026',
            PipelineSchedule.pipeline_uuid == 'agent_dev_seed_pipeline_026',
        ).all()
