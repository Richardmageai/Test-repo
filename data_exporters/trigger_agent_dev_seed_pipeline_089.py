...
def trigger_seeded_downstream(*args, **kwargs) -> None:
    ...
    try:
        trigger_pipeline(
            'agent_dev_seed_pipeline_089',
            variables={
                'seeded_by': 'agent_aws_dev_slot_seed',
                'source_pipeline_uuid': 'agent_dev_seed_pipeline_044',
            ...
            parent_pipeline_uuid='agent_dev_seed_pipeline_044',
            schedule_name='Agent dev dependency - agent_dev_seed_pipeline_044 to agent_dev_seed_pipeline_089',
            verbose=False,
            _should_schedule=True,
        )
    finally:
        dependency_schedules = PipelineSchedule.repo_query.filter(
            PipelineSchedule.name == 'Agent dev dependency - agent_dev_seed_pipeline_044 to agent_dev_seed_pipeline_089',
            PipelineSchedule.pipeline_uuid == 'agent_dev_seed_pipeline_089',
        ).all()
